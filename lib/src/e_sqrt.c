...
#if !__TI_HAS_BUILTIN_SQRT64 && !defined(__TMS320C28XX_FPU32__)
_DATA_ACCESS static const __ieee64_t	one	= 1.0, tiny=C(1.0e-300);
#endif
...
F(sqrt)(__ieee64_t x)
{
    ...
    return __ti_builtin_sqrt64(x);
#elif defined(__TMS320C28XX_FPU32__) && !defined(__TMS320C28XX_FPU64__)
    /* x < 0 is a domain error. */
    if (x < 0) { __raise_invalid(); return NAN; }
    ...
    float xf = x;
    float x0f = __eisqrtf32(x);

    /* for speed, do most of the iterations in 32-bit float */

    int i;
    for (i = 0; i < 3; i++)
      x0f *= (1.5 - xf * 0.5 * x0f * x0f);

    /* switch to 64-bit for accuracy */
    ...
    /*
     * We'll compute 1/sqrt(x) using the Newton-Raphson method from an
     * initial approximation using EISQRTF64 (SPRUHS1B).  The
     * number of iterations used here may not be optimal for this
     * instruction.
     *
     * n.b. __eisqrtf64 cannot generate -0, NaN, or denormal values
     *
     * n.b. this loop is divergent for very small x
     */

    double x0 = __eisqrtf64(x);
    int i;
...
long double sqrtl(long double x) __attribute__((__alias__("sqrt")));
#endif
